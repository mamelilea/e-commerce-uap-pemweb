<x-guest-layout>
    ...
    <div class="text-center mb-8">
        <h2 class="font-sans text-2xl font-bold text-hubbub-pink uppercase tracking-widest mb-2">LOGIN</h2>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-6">
        ...
        <div class="pt-4">
            <button type="submit" class="w-full bg-hubbub-pink text-white font-sans font-bold py-3.5 rounded-xl hover:bg-pink-600 transition-all shadow-md hover:shadow-lg uppercase tracking-widest text-sm transform hover:-translate-y-0.5">
                {{ __('Login') }}
            </button>
        </div>
        ...
        <div class="flex items-center justify-center space-x-2 text-xs font-sans mt-8 font-medium">
             @if (Route::has('password.request'))
                <a class="text-gray-400 hover:text-hubbub-pink transition-colors" href="{{ route('password.request') }}">
                    {{ __('Forgot password?') }}
                </a>
            @endif
             <span class="text-gray-300">·</span>
            <a class="text-hubbub-pink hover:text-pink-700 font-bold uppercase tracking-wide" href="{{ route('register') }}">
                {{ __('Register now') }}
            </a>
        </div>
    </form>
