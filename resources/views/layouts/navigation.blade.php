<nav x-data="{ open: false }" class="bg-black border-b border-gray-900">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('home') }}">
                        <x-application-logo class="block text-2xl text-white" />
                    </a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(Auth::user()->isSeller())
                        <x-nav-link :href="route('seller.dashboard')" :active="request()->routeIs('seller.dashboard')" class="text-gray-300 hover:text-white">
                            {{ __('Seller Dashboard') }}
                        </x-nav-link>
                    @else
                        <!-- Option to become a seller if not one -->
                        <x-nav-link :href="route('seller.register')" :active="request()->routeIs('seller.register')" class="text-gray-300 hover:text-white">
                            {{ __('Open Store') }}
                        </x-nav-link>
                    @endif
                </div>
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-gray-200 hover:bg-gray-900 focus:outline-none focus:bg-gray-900 focus:text-gray-200 transition duration-150 ease-in-out">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-gray-900">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('home')" :active="request()->routeIs('home')" class="text-gray-300 hover:text-white hover:bg-gray-800">
                {{ __('Home') }}
